        ...
        #for i in range(50):
            ...
            if ret == True:
                ...
                for (x,y,w,h) in faces:
                    ...
                    new_image = "images/" + str(k) + ".png"
                    rc, jpg = cv2.imwrite(new_image, frame[y:y+h,x:x+w])
####################################################################################
...
####################################################################################
                    images_to_publish.append(new_image)
        ...
        print("Finished faces loop")

        for image in images_to_publish:
            im = Image.open(image)
            imbytearr = image_to_byte_array(im)
            mqttclient.publish("local/faces", payload=imbytearr)
